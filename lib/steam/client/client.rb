...
module Steam
  class Client
    ...
    def get_best_server
      ...
      get_servers_list.map(&:value).compact.map(&score_server).sort_by { |e| e[:score] }.first[:url]
    end
    ...
    def query_server_load(server)
      load = 110
      url = "https://#{server}/cmping/"
      logger.debug "Checking #{server}"
      latency = Benchmark.measure do
        load = RestClient::Request.execute(method: :get, url: url, timeout: 1).headers[:x_steam_cmload]
      end
      { url: url, latency: latency.real * 1000, load: load.to_i}
